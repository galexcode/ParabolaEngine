...
public:
	LightingHolder(const ltbl::AABB &region, sf::RenderWindow *window) : myLightSystem(region, window){
...
void LightingEffects::create(BoundingBox &area, Window *window){
	...
	myData = new LightingHolder(ltbl::AABB(ltbl::Vec2f(0, 0), ltbl::Vec2f(area.Size.x , area.Size.y)), window);
	myReady = true;
	myBounds = area;
};
